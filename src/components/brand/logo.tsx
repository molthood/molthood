...
/**
 * The mark and the wordmark, identical on every surface.
 *
 * There is no `onDark` prop. The wordmark reads `--color-wordmark`, which the
 * `.molthood-dark` scope redefines — so the same component is legible on the
 * field and on black without any caller knowing which one it is standing on.
 * The colour has to change (5.99:1 on the field, 2.13:1 on black for the deep
 * green), but that is the palette's job, not the call site's.
 */
